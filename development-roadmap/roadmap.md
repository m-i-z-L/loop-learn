...
> 最終更新: 2026-04-30 (M6 デッキ単位復習 完了・反映)
...
- [x] **Mermaid対応** — MermaidRenderer (useEffect + mermaid.render()) / CardPreview に next/dynamic(ssr:false) で統合 / mermaid.initialize はモジュールスコープで1回のみ実行 (`.steering/20260422-mermaid対応`)
- [x] **ダッシュボードスタブ** — `/dashboard` 最小ページ (今日の復習枚数表示 + disabled 復習ボタン) / AppNav ダッシュボードリンク追加 (`.steering/20260423-ダッシュボードスタブ`)
- [x] **SM-2サービス** — SM2Service.calculate / EF更新 (ΔEF=0.1×(rating-3)) / インターバル計算 / 12テストケース100%カバレッジ (`.steering/20260423-SM-2実装`)
- [x] **復習API** — reviewCard service / POST /api/cards/[cardId]/review / Prisma $transaction (Card更新+ReviewLog作成) / Zod バリデーション (`.steering/20260423-SM-2実装`)
- [x] **復習セッションUI** — ReviewCard (CSS flipアニメーション) / RatingButtons / SessionProgress / ReviewSession / /review + /review/summary ページ / ダッシュボードボタン有効化 (`.steering/20260424-復習セッションUI`)
- [x] **学習進捗ダッシュボード** — StatsService (getUserStats / getHeatmapData) / GET /api/stats / HeatmapCalendar・MasteryDistribution・WeeklyBar コンポーネント / /stats ページ / AppNav 進捗リンク追加 (`.steering/20260424-学習進捗ダッシュボード`)
- [x] **AI問題自動生成** — Gemini 2.5 Flash 連携 / generateCardsSchema (Zod) / generateCards service / POST /api/ai/generate / GenerateForm・GeneratedCardItem・GeneratedCardList / /ai/generate ページ / AppNav AI生成リンク追加 (`.steering/20260425-AI問題自動生成`)
- [x] **デッキ単位復習 UX** — DeckCard に「復習する (X枚)」ボタン追加 / 復習ページのデッキ名・アイコン表示 / サマリーページに「このデッキに戻る」リンク / ダッシュボードにデッキ別復習リスト (`.steering/20260430-デッキ単位復習`)

### 進行中

なし（M6 デッキ単位復習完了。残タスク: メール認証・PWA・E2E・ページネーション）
...
| M2 | デッキ & カード管理 | デッキCRUD・カード一覧/編集/削除・Mermaid対応・ダッシュボードスタブ | カード作成数 (50枚/月) | ✅ 完了 |
| M3 | 復習エンジン | SM-2フル実装・復習セッションUI・自己評価 | 7日継続率・復習完了率・セッション数 | ✅ 完了 |
| M4 | 学習進捗ダッシュボード | ヒートマップ・ストリーク・習熟度分布・週次グラフ | 7日継続率・NPS | ✅ 完了 (2026-04-23) |
| M5 | AI問題自動生成 | Gemini API連携・カード一括生成UI・ドメイン指定 | MAU・カード作成数 | ✅ 完了 (2026-04-25) |
| M6 | MVP品質完成 | デッキ単位復習・E2Eテスト・PWA対応・メール認証 | 7日継続率・UX品質 | 🚧 進行中 (デッキ単位復習 ✅) |
| M7 | 学習モード拡張 | 精緻的質問モード・インターリービング学習 | 復習完了率・NPS | ⬜ 未着手 |
| M8 | コンテンツ入力強化 | コーネルノート連携・学習リマインダー通知 | 7日継続率・セッション数 | ⬜ 未着手 |
| M9 | ソーシャル & チャレンジ | 公開デッキ・コードチャレンジ形式 | MAU・カード作成数 | ⬜ 未着手 |
...
> M3とM5は依存関係がないため並行開発が可能。M7以降はM3の復習エンジンを前提とする。
...
| Gemini API (2.5 Flash) | AI問題自動生成 | M5〜 | `GEMINI_API_KEY`、無料枠15RPM・100万トークン/日 |
...
- [M6詳細](milestones/M6-mvp-quality.md)
- [M7詳細](milestones/M7-learning-modes.md)
- [M8詳細](milestones/M8-content-input.md)
- [M9詳細](milestones/M9-social-challenge.md)
