...
> 状態: 🚧 進行中 (SM-2サービス・復習API完了、復習セッションUI未着手)
> 対象期間: 目安 1〜2週間
...
- [x] `src/lib/services/review.ts` を作成
  ...
  - [ ] `getTodayReviewCards(userId, deckId?)` — `nextReviewDate <= today` のカードを取得 (復習UI実装時)
  - [ ] `getSessionSummary(userId, sessionDate)` — セッション終了サマリー用統計 (復習UI実装時)
...
- [x] `tests/unit/lib/services/sm2.test.ts` (12ケース・100%カバレッジ)
  - [x] Rating 1〜4 の各計算結果の正確性
  - [x] easeFactor の下限 (1.3) ・上限 (2.5) のクランプテスト
  - [x] nextReviewDate の日付計算精度テスト
- [x] `tests/unit/lib/services/review.test.ts`
  - [x] reviewCard のCard更新・ReviewLog作成のテスト (Prismaモック)
  - [x] カード不存在・他ユーザーアクセス → null を返すテスト

### 品質チェック (SM-2フェーズ)

- [x] `npm run lint` がパスする
- [x] `npm run typecheck` がパスする
- [x] `npm test` がパスする (45テスト全通過)

### 品質チェック (復習セッションUIフェーズ — 未着手)
